...
class VariationalBGMM:
    ...
    def mv_normal_logp(self, value, mu, Sigma):
        ...
        diff = value - mu
        logp = -0.5 * n * np.log(2 * np.pi) - 0.5 * np.log1p(np.linalg.det(Sigma)) - 0.5 * np.sum(
            diff.dot(Sigma_inv) * diff, axis=1)
        return logp
    ...
    def update_assignments(self):
        for _ in range(self.num_iterations):
            ...
            self.q_z = np.exp(self.q_z - self.q_z.max(axis=1, keepdims=True))
            self.q_z /= self.q_z.sum(axis=1, keepdims=True)
...
class VariationalBGMM:
    ...
    def mv_normal_logp(self, value, mu, Sigma):
        ...
        Sigma_inv = np.linalg.inv(Sigma)
        diff = value - mu
        logp = -0.5 * n * np.log(2 * np.pi) - 0.5 * np.log(np.linalg.det(Sigma)) - 0.5 * np.sum(diff.dot(Sigma_inv) * diff, axis=1)
        return logp
    ...
    def fit(self, num_iterations=1000):
        ...
        with pm.Model() as model:
            ...
            # Here, set self.num_clusters to be a shared variable so that it can be modified
            # This is to allow changing the number of clusters during the sampling process
            num_clusters_shared = pm.Data('num_clusters_data', self.num_clusters, mutable=True)
